document.addEventListener('DOMContentLoaded', () => {
    ...
    function subscribeToChannel(channelId) {
        // Si un abonnement existe déjà, on se désabonne avant de créer le nouveau
        if (echoChannel) {
            echoChannel.unsubscribe();
        }

        echoChannel = window.Echo.private(`channels.${channelId}`)
            .listen('.MessageSent', (event) => {
                // Ajoute le message reçu et met à jour l'affichage
            ...
            connection.bind('message', (data) => {
                ...
                if (data && data.event === "GroupAccessUpdated") {
                    ...
                    if (action === 'added' && !existingBtn) {
                        ...
                        // Cas de suppression : le bouton existe et doit être retiré
                        existingBtn.remove();
